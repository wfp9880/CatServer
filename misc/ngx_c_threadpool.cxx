...
void* CThreadPool::ThreadFunc(void* threadData)
{
    ...
    while(true)
    {
        ...
        //必须要用while这种写法，因为：pthread_cond_wait()是个值得注意的函数，调用一次pthread_cond_signal()可能会唤醒多个【惊群】【官方描述是 至少一个/pthread_cond_signal 在多处理器上可能同时唤醒多个线程】
        ...
        if( jobbuf == NULL && m_shutdown == false)
        {
            ...
            //pthread_cond_wait()阻塞调用线程直到指定的条件有信号（signaled）。
                //该函数应该在互斥量锁定时调用，当在等待时会自动解锁互斥量。在信号被发送，线程被激活后，互斥量会自动被锁定，当线程结束时，由程序员负责解锁互斥量。  
...
/*
    ...
    if(ifallthreadbusy == false)
    {
        if(irmqc > 5)
        {
            //如果有空闲线程，并且 接收消息队列中超过5条信息没有被处理，可能唤醒丢失
