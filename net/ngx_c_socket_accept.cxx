...
void CSocekt::ngx_event_accept(lpngx_connection_t oldc)
{
    ...
    {     
        ...
        if(s == -1)
        {
            ...
            {
                //除非你用一个循环不断的accept()取走所有的连接，不然一般不会有这个错误【这里只取一个连接，也就是accept()一次】
                return ;
            } 
            level = NGX_LOG_ALERT;
            ...
            {
                //do nothing这里目前先不处理吧【因为上边已经写这个日志了】；
            }            
            return;
        }  //end if(s == -1)
        ...
        {
            //比如允许同时最大2048个连接，但连接池却有了 2048*5这么大的容量，这肯定是表示短时间内 产生大量连接/断开，因为我们的延迟回收机制，这里连接还在垃圾池里没有被回收
